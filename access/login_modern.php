...
function message_box($msg_type, $msg_body){
...
function defaultCompany()
{
    ...
    if ($login_timeout) { //  Time out
        message_box("ERROR",_('Authorization timeout'));
    }
    // Start Login form
    echo '<div class="main">'. "\r\n";
    echo '<div class="login-form">';
    echo "<h1>" . _("Member Login"). "</h1>\n";
    echo'<div class="head">' . "\r\n";
    echo '<img src="' .$path_to_root.'/themes/default/images/logo_iulerp.png" alt=""/></div>' . "\r\n";
    ...
    echo '<input name="user_name_entry_field" size="20" type="text" class="text" value="' ._('User name'). '" onfocus="this.value = \'\';" onblur="if (this.value == \'\') {this.value = \''._('User name') .'\';}">' ."\n";

	$password = $allow_demo_mode ? "password":"";
	//password_row(_("Password:"), 'password', $password);
    echo '<input type="password" name= "password" value="'. $password .'" " onfocus="this.value = \'\';" onblur="if (this.value == \'\') {this.value = \''. $password .'\';}">' ."\n";
	...
	if ($login_timeout) {
		...
		if (!@$text_company_selection) {
            ...
            echo '<input DISABLED type="text" value="' ._("Company") . ">\r\n";

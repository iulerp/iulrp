	...
	class renderer
	{
		...
		function menu_header($title, $no_menu, $is_index)
		{
			...
			if (!$no_menu)
			{
				...
				$img = "<img src='$local_path_to_root/themes/studio/images/login.gif' width='14' height='14' border='0' alt='"._('Logout')."'>&nbsp;&nbsp;";
				$himg = "<img src='$local_path_to_root/themes/studio/images/help.gif' width='14' height='14' border='0' alt='"._('Help')."'>&nbsp;&nbsp;";
				echo "<div id='header'>\n";
				...
				echo "  <a class='shortcut' href='$path_to_root/admin/change_current_user_password.php?selected_id=" . $_SESSION["wa_current_user"]->username . "'>" . _("Change password") . "</a> | \n";
			 	if ($help_base_url != null)
					echo "  <a target = '_blank' onclick=" .'"'."javascript:openWindow(this.href,this.target); return false;".'" '. "href='". 
						help_url()."'>$himg" . _("Help") . "</a> | \n";
				echo "  <a class='shortcut' href='$path_to_root/access/logout.php?'>$img" . _("Logout") . "</a>";
				echo "</div>\n";
				$indicator = "$path_to_root/themes/".user_theme(). "/images/ajax-loader.gif";
				echo "<h2 id='logo-text'>$power_by $version<span style='padding-left:280px;'><img id='ajaxmark' src='$indicator' align='center' style='visibility:hidden;'></span></h2>\n";
				echo "</div>\n"; // header
				echo "<div id='_tabs'>\n";
				echo "<ul>\n";
		...
		function menu_footer($no_menu, $is_index)
		{
			...
			if ($no_menu == false)
			{
				...
				{
					echo "<span><a target='_blank' href='$power_url'>$power_by $version</a></span>\n";
					echo "<span>".Today() . "&nbsp;" . Now()."</span>\n";
					echo "<span>" . $db_connections[$_SESSION["wa_current_user"]->company]["name"] . "</span>\n";
					echo "<span>" . $_SERVER['SERVER_NAME'] . "</span>\n";
					echo "<span>" . $_SESSION["wa_current_user"]->name . "</span>\n";
					echo "<span>" . _("Theme:") . " " . user_theme() . "</span>\n";
					echo "<span>" . show_users_online() . "</span>\n";
				}
				echo "</div>\n"; // footer
			}
			echo "</div>\n"; // wrap

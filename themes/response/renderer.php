...
// Author: Joe Hunt, 17/11/2015. Upgraded to release 2.4. 10/11/2015.
	...
	class renderer
	{
		...
		function menu_header($title, $no_menu, $is_index)
		{
			...
			if (!$no_menu)
			{
				//echo "<script type='text/javascript'> if (!('ontouchstart' in document.documentElement)) {document.documentElement.className += ' no-touch';}</script>\n";
				...
				foreach($applications as $app)
				{
                    if ($this->check_application_access($app))
                    {
						...
						if ($n)
							$class = "class = 'has-sub'";
						if ($account[3] == $app->id)
							$class = ($n) ? "class='has-sub active'" : "class='active'";
						echo "  <li $class><a href='$local_path_to_root/index.php?application=" . $app->id
							."'$acc[1]>" . $acc[0] . "</a>\n";
						if (!$n)
						{
							echo "  </li>\n";
						...
						foreach ($app->modules as $module)
						{
							...
							foreach ($apps2 as $application)	
							{
								...
								elseif (!$this->hide_inaccessible_menu_items())	
									echo "          <li><a href='#'><font color='gray'>$lnk[0]</font></a>/li>\n";
							}
							if ($n)
								echo "        </ul>\n";	
							echo "      </li>\n";
